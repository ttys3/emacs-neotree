  ...
  (let ((n-insert-image (lambda (n)
    ...
    (cond
     ...
     ((equal neo-theme 'icons)
      (unless (require 'icons-in-terminal nil 'noerror)
        (error "Package `icons-in-terminal' isn't installed"))
      (setq-local tab-width 1)
      (or (and (equal name 'open)  (funcall n-insert-symbol " \t"))
          (and (equal name 'close) (funcall n-insert-symbol " \t"))
          (and (equal name 'leaf)  (funcall n-insert-symbol (format "%s\t" (icons-in-terminal-icon-for-file node-name))))))
     (t
      (or (and (equal name 'open)  (funcall n-insert-symbol "- "))
          (and (equal name 'close) (funcall n-insert-symbol "+ ")))))))
